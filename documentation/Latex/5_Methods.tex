...
\end{itemize}
A more complete list of thermodynamic properties can be found in ref. \cite{SpanBook}. For the $NVE$ ensemble, there are two sets of $A^\text{r}_{nm}$ output values in \textit{ms}2. Each set corresponds to a different statistical mechanical entropy definition.% \cite{Lus12}. 
...
\end{equation}
To obtain the total isobaric heat capacity $c_\text{p}$, the solely temperature dependent ideal gas contribution $c^{\text{id}}_{p}\left(T\right)$ has to be added. This ideal property is accessible e.\,g. by quantum chemical methods and can often be found in data bases \cite{rowley}.
...
An extended time step is defined in \textit{ms}2 for the calculation of the fluxes, the autocorrelation functions and their integrals, which may be significantly times longer than the MD time step. As a consequence, the memory demand for the autocorrelation functions was reduced and the restart files, which contains the current state of the autocorrelation functions and time integrals, become accordingly smaller.
\newline \newline
\textbf{Diffusion coefficients:}\\
The self-diffusion coefficient $D_{i}$ is related to the mass flux of single molecules of component $i$ within a fluid. Therefore, the relevant Green-Kubo expression is based on the individual molecule velocity autocorrelation function \cite{gubbins1}
\begin{equation}
...
\label{eq:difcoef}
\end{equation}
where $N$ is the total number of molecules. Because of Onsager's reciprocity theorem, $L_{ij}=L_{ji}$ for $i\neq j$. In this context, the Maxwell Stefan diffusion coefficient for binary mixtures is given by \parencite{krishna}
\begin{equation}
...
\end{equation}
Five independent terms of the stress tensor $J_p^{xy}$, $J_p^{xz}$, $J_p^{yz}$, $(J_p^{xx}-J_p^{yy})/2$ and $(J_p^{yy}-J_p^{zz})/2$ were considered to improve statistics~\cite{Alfe1998}. The component $J^{xy}_{p}$ of the microscopic stress tensor $\textbf{J}_{p}$ is given in terms of the molecular positions and velocities by \cite{hoheisel}
\begin{equation}
...
In the present release of $ms$2, the RDF can be calculated during MD and MC simulation runs for pure components and mixtures on the fly. The RDF is sampled between all Lennard-Jones sites. In order to evaluate RDF's for arbitrary positions, say point charge sites, superimposed dummy Lennard-Jones sites with the parameters $\sigma = \epsilon = 0$ have to be introduced in the potential model file by the user.
...
\begin{figure}[htb]
	...
	\label{fig:Hbonding}
\end{figure}
% Formulas in fig_HBonding.tex .. Simon's fault
...
\end{itemize}
As the H-bonding analysis is implemented in \textit{ms}2, charge-charge-charge configurations relating to two components (which can be one and the same) can be sampled. Not necessarily Oxygen and Hydrogen atoms have to be part of these structures. The implemented geometric criterion evaluates the triangle between three charge sites of two molecules -- either two sites in the acceptor molecule or two sites in the donor molecule -- and compares these with the criterion thresholds. \\
...
Fig. \ref{fig:Hbonding} visualises these parameters. The first six parameters declare which charge -- charge interactions of which component are to be considered by \textit{ms}2. The acceptor and donor sites listed in the \texttt{*.par} file have to be point charges listed in the \texttt{*.pm} file. AccComp and DonComp are the component numbers of the components that are supposed to be the acceptor and donor of the H-bond, respectively. Both have to be integer values according to the order of the components listed in the \texttt{*.par} file. The four charge sites that participate in the H-bond have to be set by the user as AccAccSite, AccDonSite, DonAccSite and DonDonSite. They have to be integer values according to the order of the charge sites listed in the \texttt{.*pm} file. AccAccSite and AccDonSite are the acceptor and donor charge sites in the component that is acting as the acceptor molecule in the H-bond while DonAccSite and DonDonSite are the acceptor and donor charge-sites in the donor molecule. As mentioned, only three charge sites are needed for the criterion, thus either the AccDonSite or the DonDonSite has to be set to \texttt{0} by the user as can be seen in table \ref{tab:Hbonding}. This is equivalent of defining either $\ell_\mathrm{DA}$ and $\varphi_\mathrm{DAA}$ or $\ell_\mathrm{AD}$ and $\varphi_\mathrm{AAD}$.\\
The last three numeric parameters in table \ref{tab:Hbonding} are the actual geometric length $\ell_\mathrm{AA}$, $\ell_\mathrm{AD}$ or $\ell_\mathrm{DA}$ and angles $\varphi_\mathrm{DAA}$ or $\varphi_\mathrm{AAD}$ as described in \parencite{Haughney1987}. \textit{ms}2 always expects these values to be specified in \AA ngstrom and degree, respectively, independent of the settings of the \texttt{Units} keyword in the \texttt{*.par} file before.

Table \ref{tab:Hbonding} gives two examples for a complete parameter set to be specified in a \texttt{*.par} file.
\begin{table}[h!]
	\centering
	\caption{Example of Parameter sets for H-bonding criterion.}
	...
	\label{tab:Hbonding}%
\end{table}%
During simulation, \textit{ms}2 lists the analysis of the H-bonding in the \texttt{*.rav} file. 
Following the \textit{Haughney}-criterion, \textit{ms}2 calculates and itemizes the number $N_i$ of molecules with $i=0,1,2,3$ or more hydrogen bonds. In the \texttt{*.rav} file these numbers are listed according to the component combination. E.g. the \texttt{N3\_( 1, 2, 2, 3)} column lists the number of molecules which correspond to component 1 and are bonded twice with component 2 and once with component 3. As usual for the \texttt{*.rav} file, these values are running averages. 
...
The chemical potential of component $i$ can be separated into the temperature $T$ dependent part $\tilde{\mu}^{\mathrm{id}}_{i}(T)$ of the ideal gas contribution to the chemical potential of the pure component and the remaining contribution $\tilde{\mu}_{\mathrm{i}}(T,\textbf{x},v)-\tilde{\mu}^{\mathrm{id}}_{i}(T)$which is often referred to as the configurational chemical potential. The solely temperature dependent ideal gas contribution $\tilde{\mu}^{\mathrm{id}}_{i}(T)$ cancels out in the calculation of phase equilibria and is therefore not implemented in \textit{ms}2. The chemical potential depends on the real substance behavior due to the molecular interactions and can be determined with \textit{ms}2 using two different techniques: Widom's particle insertion method and thermodynamic integration. 
\newline \newline 
\textbf{Widom's particle insertion method:}\\
Widom's particle insertion method is a conceptually straightforward approach to calculate the chemical potential of a component $i$ and was presented by Widom \cite{widom}. It allows for a determination of the chemical potential with low computational cost, both for pure substances and mixtures. For a three-site Lennard-Jones fluid, the computational demand is shown in Table~\ref{tab:ComputationalDemand}. The execution time for the determination of the chemical potential increases roughly linearly with the specified number of test molecules that are sampled.
\begin{table}[h!]
	\centering
...
The value of $U_{N+1}$ is highly dependent on the random position or orientation of the test molecule. In addition, the density of the system has a significant influence on the accuracy of the calculation. For very dense fluids, test molecules almost always overlap with some real molecules, which leads to a potential energy $U_{N+1}\rightarrow \infty$ and thus to no contribution to Eq.~\eqref{eq:TestMoleculeInsertion} resulting in poor statistics for the chemical potential or even complete failure of the sampling. Within limits, lower statistical uncertainties of the chemical potential can be achieved by inserting a large number of test molecules into the simulation volume, which leads to an increasing computational demand. Nonetheless, the Widom's particle insertion method certainly fail above some density or test molecule size.
\newline \newline
\textbf{Thermodynamic integration:}\\
Thermodynamic integration is one solution to overcome the limitations of Widom's particle insertion method. The idea behind calculating the chemical potential using thermodynamic integration is to avoid insertion of the test particle in system $A$ and perform it in system $B$ for which it is not problematic or practically not required at all. Since the chemical potential is a state variable, its change $\mu_A-\mu_B$ can be calculated using any arbitrary path between state $A$ and $B$. This path is represented by the parameter $\lambda$. It can be shown \cite{Frenkel} that the relation between $\lambda$ and $\mu_A-\mu_B$ is 
\begin{equation}
...
\begin{itemize}
  ...
  \item \texttt{LambdaExponent} (the exponent $x$ for the non-linear scaling $U_j(\lambda)=\lambda^x U_j$ $(\lambda_\text{min} \leq \lambda \leq 1)$, default value is 4.0 (it does not have to be an integer)).
\end{itemize}
One should keep in mind when choosing these parameters that thermodynamic integration always involves larger statistical uncertainties than the equivalent pure Widom scenario for a state point for which the latter works perfectly. In other words: the larger the contribution of the numerical integration part is compared to the Widom contribution at $\lambda_\text{min}$ is, the higher the statistical uncertainty of the calculated chemical potential gets. This means that setting $\lambda_\text{min}$ very low or the exponent $x$ very high (e.g. 12) might not be beneficial in many systems. Moreover, high exponent $x$ values for MD simulations are not advised since the scaling with $x$ at high $lambda$ values (where the absolute value of $U_j(\lambda)$ is close that of the non-scaled state) may result in new $U_j(\lambda)$ energies (thus forces) that non-negligibly interfere with the dynamics of the system when $\lambda$ is changed during the simulation. For MD simulations, a value smaller then 1.0 for $x$ is possibly a better choice for many systems.
...
\end{equation}
can be straightforwardly obtained by Widom's particle insertion method \parencite{Sindzingre_1987} and their calculation is a feature of \textit{ms}2. $U_N$ is the potential energy of the $N$ particle system, while $U_{N+1}$ is that of the test particle. The brackets <> denote $NpT$ ensemble averages. For thermodynamic integration, the calculation of these properties is currently not implemented.
