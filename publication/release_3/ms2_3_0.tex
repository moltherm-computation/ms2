...
\begin{abstract}
A new version release (3.0) of the molecular simulation tool {\it ms}2 [S. Deublein et al., Comput. Phys. Commun. 182 (2011) 2350 and C. Glass et al., Comput. Phys. Commun. 185 (2014) 3302] is presented. Version 3.0 of {\it ms}2 features two new ensembles ($NVE$ and $NpH$); the calculation of Helmholtz energy derivatives in the $NVE$ ensemble, the osmotic pressure and the Quaternary Maxwell-Stefan diffusion coefficients; thermodynamic integration as a method for calculating the chemical potential; the detection of hydrogen bonding during simulation as well as the ability to sample an arbitrary number of state points in a single run.
...
\noindent
{\em Summary of revisions:} two new ensembles ($NVE$ and $NpH$), new properties (Helmholtz energy derivatives, chemical potential via thermodynamic integration, osmotic pressure, Quaternary Maxwell-Stefan diffusion coefficients), new functionalities (detection of hydrogen bonding, the ability to sample an arbitrary number of state points in a single run)\\
...
\noindent
{\em Restrictions:} typical problems addressed by $ms2$ are solved by simulating systems containing typically 1000 -- 4000 molecules that are modelled as rigid bodies\\
...
%% main text
\section{New Ensembles ($NVE$ and $NpH$) and Helmholtz energy derivatives}
\label{sec:ensembles}

An ensembles is the set of all theoretically possible microscopic configurations (configurations on the molecular level) under some specific macroscopic conditions. The microcanonical ensemble ($NVE$) is the set of all configurations that fulfills the condition of having the same number of particles $N$, volume $V$ and energy $E$, whereas the isobaric-isoenthalpic ensemble ($NpT$) represents a system at constant number of particles $N$, pressure $p$ and enthalpy $H$. Molecular dynamics (MD) simulate the time evolution of the system by numerically solving the Newton's equations of motion for each molecule. Because of to the nature of this solution, the time is discredized and the method yields microscopic configurations at discrete but consecutive time steps. The Monte Carlo (MC) method is the application of statistical mechanics to describe molecular systems. With this approach, microscopic configurations are generated and potentially accepted such a way that only relevant and physically meaningful configurations are created. The generation and acceptance of these configuration is governed by a specific probabilities \cite{fre02} that are ensemble dependent and known for essentially every relevant ensemble. $ms2$ For MC simulations, the $NVE$ and $NpH$ ensembles were implemented in $ms2$ 3.0 according to Ref. \cite{lus98} and \cite{kri05}, respectively. For MD simulations, the pressure is kept constant using Andersenʼs barostat \cite{and80} for the $NpH$ ensemble. Because the solution of Newton's equations of motion is approximate, the total energy of the system $E=K+U$, which consits of a kinetic $K$ (exclusively molecular velocity dependent) and potential $U$ (exclusively spacial coordinate dependent) contribution, is not rigorously conserved in a standard $NVE$ run. Therefore, the velocity (translational and rotational) of every molecule is rescaled such that their sum calculated by the equipartition theorem matches $E-U$, where $E$ is specified and $U$ is calculated from the current configuration. For an $NpH$ run, the solution is analogous: velocities are rescaled such that the current kinetic energy $K$ matches $H-U-pV$, where $H$ and $p$ are specified, $U$ and $V$ are dependent on the current microscopic configuration. This extends the available ensembles to five:  $NVT$, $\mu VT$, $NVE$, $NpT$ and $NpH$, where $\mu$ is the chemical potential.\\ Table~\ref{tab:ensembles} contains numerical results for state points state points located in the dens fluid region for verification purposes.
...
\end{equation}
with the molar residual Helmholtz energy $f^\text{r}$, the temperature $T$, the density $\rho$, the molar gas constant $R$ were also introduced for the $NVE$ ensemble up to the order $n=3$ and $m=2$. It can be shown that the total $f/(RT)$ can be additively separated into an ideal $f^\text{o}/(RT)$ and a residual part$f^\text{r}/(RT)$. The ideal part by definition corresponds to the value of $f/(RT)$ when no intermolecular interactions are at work \cite{spa00}. $f^\text{o}/(RT)$ consist of an exclusively temperature and an exclusively density dependent part. The former has a trivial density dependence $\ln(\rho/\rho_\text{ref})$, whereas the latter has a non-trivial temperature dependence. The formalism that allows for the calculation of these derivatives on the fly from a single simulation run per state point was published by Lustig \cite{lus11, lus12} and was already introduced for the $NVT$ ensemble in the previous release. $ms2$ 3.0 now yields these derivatives also for $NVE$ simulations. Here, however, in contrast with $NVT$ an simulation, there are two numerical result for each calculated derivative $A_{nm}$ which is due to the two possible entropy definition in statistical mechanics \cite{lus12}. In any case, these two different sets of results must be identical in the thermodynamic limit ($N \longrightarrow \infty$). In practice, they already agree within their estimated statistical uncertainty for a simulation with around a thousand particle.\\
...
\begin{table}
  ...
  \caption{Comparison of the ensembles implemented in \textit{ms}2 for a dense liquid state point of Methyl Fluoride (R41). Statistical uncertainties were estimated with the method of Flyvbjerg and Petersen \cite{fly89}}
    ...
    \hline
          &       & \textbf{Temperature} & \textbf{Density} & \textbf{Pressure} & \textbf{Potential energy} \\
          &       & K   & mol/l & MPa & kJ/mol \\
    \hline
    \multicolumn{1}{c}{\multirow{4}[0]{*}{\textbf{MD}}} & \multicolumn{1}{l}{\textit{NVT}} & 220   & 27    & 100.2 $\pm$ 0.2 & -16.63 $\pm$ 0.02 \\
    ...
    \multicolumn{1}{c}{\multirow{4}[0]{*}{\textbf{MC}}} & \multicolumn{1}{l}{\textit{NVT}} & 220   & 27    & 100.2 $\pm$ 0.1 & -16.66 $\pm$ 0.01 \\
...
The previously described method of Lustig does not allow for the direct sampling of the chemical potential thus entropic properties like $A_{00}$. Such an effort still requires techniques based on free energy calculation such as particle insertion and/or thermodynamic integration \cite{fre02}. Widom's particle insertion method \cite{wid63} is a conceptually straightforward approach to calculate the chemical potential with low computational cost, both for pure substances and mixtures. The total chemical potential for a particle $i$ can be separated into an ideal (o) and a residual (r) part the same way the Helmholtz energy is decomposed (cf. section~\ref{sec:ensembles}). $\mu^\text{total}_{i} (T,\rho,x_i) = \mu^\text{o}_{i}(T) + RT \ln (N_i/V/\rho_\text{ref}) + \mu^{r}_{i} (T,\rho,x_i)$, where $N_i$ is the number of particles with the same identity that of $i$, $\rho=N/V$, $x_i=N_i/N$ and $\rho_\text{ref}$ is some arbitrary reference density. The part $\mu^\text{total}_{i} - \mu^{o}_{i}(T)$ is often referred to as the configurational chemical potential $\mu^\text{conf}_{i}$. Widom's method requires the frequent insertion of an additional $(i=N+1)$ molecule into the simulation volume at a random position with a random orientation. At constant temperature and constant pressure or volume the potential energy $U_i$ of this test molecule, the interaction energy with all other "real" $N$ molecules, yields the configurational chemical potential according to
\begin{equation}
...
\label{eq:widom}
\end{equation}
where $k_\text{B}$ is Boltzmann's constant. The test molecule is removed immediately after the calculation of its potential energy $U_i$, thus it does not influence the properties of the system in any way. In contrast to the usual convention, the brackets $<>$ here have a dual meaning: They stand for either $NVT$ or $NpT$ ensemble average as well as an integral over all the possible position and orientation of the test molecule in the system. The density of the system has a significant influence on the accuracy of this method. For state points with high density, test molecules almost always overlap with some real molecules, which leads to a potential energy $U_i \rightarrow \infty$ and thus to no contribution to Eq.~\ref{eq:widom} resulting in poor statistics and often to the complete failure of the sampling.\\
Thermodynamic integration is one solution to overcome the limitations of Widom's particle insertion method. The idea behind calculating the chemical potential using thermodynamic integration is to avoid insertion of the test particle in system $A$ and perform it in system $B$ for which it is not problematic or even not required at all. Since the chemical potential is a state variable, its change $\mu_A-\mu_B$ can be calculated using any arbitrary path between state $A$ and $B$. This path is represented by the parameter $\lambda$. It can be shown \cite{fre02} that the relation between $\lambda$ and $\mu_A-\mu_B$ is 
\begin{equation}
...
\begin{equation}
 \bm{\Delta}_{ij} = (1-x_i)\left(\frac{L_{ij}}{x_j}-\frac{L_{in}}{x_n}\right)-x_i\sum^{n}_{k=1\neq i}\left(\frac{L_{kj}}{x_j}-\frac{L_{kn}}{x_n}\right),
\end{equation} 

\noindent so that its inverse matrix $\bm{B} = \bm{\Delta}^{-1}$ is related to the Maxwell-Stefan diffusion coefficients $\textit{D}_{ij}$. In the case of a quaternary mixture, the Maxwell-Stefan diffusion coefficients are then calculated using~\cite{kri05b}

\begin{align}
{\textit{D}_{14}}&=\frac{1}{B_{11}+(x_2/x_1)B_{12}+(x_3/x_1)B_{13}},\\
{\textit{D}_{24}}&=\frac{1}{B_{22}+(x_1/x_2)B_{21}+(x_3/x_2)B_{23}},\\
{\textit{D}_{34}}&=\frac{1}{B_{33}+(x_1/x_3)B_{31}+(x_2/x_3)B_{32}},\\
{\textit{D}_{12}}&= \frac{1}{1/{\textit{D}_{24}}-B_{21}/{x_2}},\\
{\textit{D}_{13}}&= \frac{1}{1/{\textit{D}_{14}}-B_{13}/{x_1}},\\
{\textit{D}_{14}}&= \frac{1}{1/{\textit{D}_{24}}-B_{23}/{x_2}},\\
\end{align}

\noindent where $x_i$ is the molar fraction of component $i$.

\section{Hydrogen bonding}
\label{sec:Hydrogen bonding}

There is no unambiguous characterization of a hydrogen bond between two molecules \cite{Stillinger80, HI10, ADKSSACCDHKLMN11}. The H~bond \qq{is a structural motif and involves at least three atoms} \cite{TK06}. IUPAC defines the H~bond as \qq{an attractive interaction between a hydrogen atom from a molecule or molecular fragment X--H in which X is more electronegative than H, and an atom or a group of atoms in the same or a different molecule, in which there is evidence of bond formation} \cite{ADKSSACCDHKLMN11}.

Energetic \cite{Stillinger80}, geometric \cite{Haughney1987}, and topological \cite{HI10} H-bonding criteria have been proposed in the literature. Geometric criteria, which are not overly complex, are often based on the following assumptions \cite{RMHH12}:
\begin{itemize}
	\item The interaction between two H-bonded sites is highly directional and short ranged.
	\item A donor interacts at most with a single acceptor, an acceptor can interact with multiple donors.
\end{itemize}
Accordingly, a class of geometric criteria for the evaluation of H-bonding networks in fluids is implemented in \textit{ms}2. Thereby, the triangle between three charge sites, situated in two molecules, is evaluated to determine whether an interaction between two sites constitutes a H bond or not. A molecule acts as a H donor to another molecule, the H acceptor, if the following conditions hold \cite{Haughney1987, RMHH12, LC93, CC06}:
\begin{itemize}
...
\end{itemize}
Therein, $\ell_\mathrm{AD}$, $\ell_\mathrm{DA}$, $\ell_\mathrm{AA}$, $\varphi_\mathrm{DAA}$, and $\varphi_\mathrm{AAD}$ are the parameters of the implemented class of H-bonding criteria; for methanol, e.g., Haughney \textit{et al.}\ \cite{Haughney1987} use $\ell_\mathrm{AD} = \ell_\mathrm{DA} = 2.6$ \AA{}, $\ell_\mathrm{AA} = 3.5$ \AA, and $\varphi_\mathrm{AAD}= \varphi_\mathrm{DAA} = 30^\circ$.
Beside methanol \cite{Haughney1987}, such geometric criteria have been applied to a variety of H-bonding fluids, in particular water \cite{LC93, CC06, LC96}, methanol-water mixtures \cite{CC14}, and ethanol \cite{RMHH12, SPG97, XYH09}, including previous molecular simulations with an unreleased development version of the \textit{ms}2 program \cite{RMHH12}. H bonds in sorbate-sorbent interactions can be treated analogously \cite{BKS14}. For hydrogen fluoride, a simple distance-based criterion can be used \cite{KBMP03}, which is also covered by the present implementation.

\begin{figure}[htb]
	\centering
	\def\svgwidth{\textwidth}
	%\input{Abb_HBonding.tex} 
	\includegraphics[width=0.5\textwidth]{Abb_HBonding_orig.eps}
	\caption{Class of H-bonding criteria implemented in the present release version of \textit{ms}2.}
	\label{fig:Hbonding}
\end{figure}
